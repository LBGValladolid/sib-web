...
### More features
- Google Analytics built in (replace `UA-xxxxxxxx-x` with your personal analytics verification key in `_includes/2021_data/head.html`)
- SEO (check `_config.yml`)
- Customized 404 Page Not Found Page
- Designed to be futureproof as you can create a subpages for each year (eg. YOUR-LINK.github.io/2015, YOUR-LINK.github.io/2021, etc)
...
## Files Structure
- The main stylesheet is stored `/css/2021_style`. Both `2021` and `2022` are sharing the same stylesheet.
- In `/css/2021_style/img` you will find where the pictures in `/2021` are stored at. You will find the images of speakers in `/css/2022_style/img`. This setup is to ensure that we can easily to move from year to year by creating new folders.
- `_2021_pages` and `_2022_pages` are the folders that store subpages.
- `_2021_data` and `_2022_data` are the folders that store each sections in home page. Those sections are can be removed by removing or commenting out the `include` code in `_layout/2021_home.html` or `_layout/2022_home.html`.
- `_data/twenty_21/` and `_data/twenty_22/` contains `data` files for agenda, faqs, home about section data, speakers, and team members information. The data file is in the format of `.yml`. The reason why the folders are named `twenty_21` and `twenty_22` are due to liquid syntax will throw errors if the name contains integer. 
